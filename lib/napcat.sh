...
generate_napcat_compose(){ local name="$1" webui_port="$2" ws_port="$3" http_port="$4" dir="$NAPCAT_BASE_DIR/$name" file="$dir/docker-compose.yml"; cat > "$file" <<EOF
services:
  ${name}:
    image: ${NAPCAT_IMAGE}
    container_name: ${name}
...
delete_napcat_instance(){ local name="$1" dir="$NAPCAT_BASE_DIR/$name" file="$dir/docker-compose.yml"; [[ -f "$file" ]] || { fail "Instance not found: ${name}"; return 1; }; $(compose_cmd) -f "$file" down || true; read -r -p "Delete data directory ${dir}? [y/N]: " ans || ans="n"; [[ "$ans" =~ ^[Yy]$ ]] && rm -rf -- "$dir"; grep -vE "^${name}\t" "$NAPCAT_INDEX_FILE" > "${NAPCAT_INDEX_FILE}.tmp" || true; mv "${NAPCAT_INDEX_FILE}.tmp" "$NAPCAT_INDEX_FILE"; success "Deleted ${name}"; }
