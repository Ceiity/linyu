...
add_napcat_instances(){ local count="$1" start max width i id name token webui ws http file total astrbot_ws_port; [[ "$count" =~ ^[0-9]+$ ]] && (( count > 0 )) || { fail "NapCat count must be a positive integer."; return 1; }; max="$(max_napcat_id)"; start=$((max+1)); total=$((max+count)); width=2; (( total >= 100 )) && width=${#total}; NAPCAT_PAD_WIDTH="$width"; for ((i=0;i<count;i++)); do id=$((start+i)); name="$(napcat_name "$id")"; token="$(random_token)"; webui="$(find_free_port $((6099+i*10)))"; ws="$(find_free_port $((3001+i*10)))"; http="$(find_free_port $((3000+i*10)))"; astrbot_ws_port="$((ASTRBOT_INTERNAL_WS_PORT + id - 1))"; step "Creating ${name}"; generate_napcat_config "$name" "$token" "$astrbot_ws_port"; file="$(generate_napcat_compose "$name" "$webui" "$ws" "$http")"; compose_pull "$file" || warn "${name} image pull failed; trying local cache."; compose_up "$file"; wait_container "$name" 180 || { docker logs "$name" --tail 100 || true; fail "${name} failed to start."; return 1; }; append_napcat_index "$name" "$webui" "$ws" "$http" "$token"; success "${name}: WebUI http://$(private_ip):${webui}/webui token=${token} reverse_ws=ws://astrbot:${astrbot_ws_port}/ws reverse_token=${ASTRBOT_REVERSE_WS_TOKEN}"; done; save_state_var NAPCAT_IMAGE "$NAPCAT_IMAGE"; sync_astrbot_platforms; }
delete_napcat_instance(){ local name="$1" dir file; dir="$NAPCAT_BASE_DIR/$name"; file="$dir/docker-compose.yml"; [[ -f "$file" ]] || { fail "Instance not found: ${name}"; return 1; }; $(compose_cmd) -f "$file" down || true; read -r -p $'\u662f\u5426\u5220\u9664\u6570\u636e\u76ee\u5f55 '"${dir}"$'\uff1f[y/N]\uff1a' ans || ans="n"; [[ "$ans" =~ ^[Yy]$ ]] && rm -rf -- "$dir"; grep -vE "^${name}\t" "$NAPCAT_INDEX_FILE" > "${NAPCAT_INDEX_FILE}.tmp" || true; mv "${NAPCAT_INDEX_FILE}.tmp" "$NAPCAT_INDEX_FILE"; sync_astrbot_platforms; success "Deleted ${name}"; }
