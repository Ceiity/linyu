...
uninstall_all(){ read -r -p "Uninstall mode: 1=keep data 2=delete all data [1/2]: " mode || mode=1; stop_all || true; while read -r f; do [[ -n "$f" ]] && $(compose_cmd) -f "$f" down || true; done < <(all_compose_files); read -r -p "Delete images ${ASTRBOT_IMAGE} and ${NAPCAT_IMAGE}? [y/N]: " imgs || imgs=n; [[ "$imgs" =~ ^[Yy]$ ]] && docker rmi "$ASTRBOT_IMAGE" "$NAPCAT_IMAGE" || true; docker network rm "$NETWORK_NAME" >/dev/null 2>&1 || true; if [[ "$mode" == "2" ]]; then rm -rf -- "$INSTALL_PREFIX"; success "Deleted $INSTALL_PREFIX"; else success "Uninstalled containers and network; data kept in $INSTALL_PREFIX"; fi; }
